...
def load_data():
    ...
    features = pd.concat(
        [feat_train, feat_val, feat_test], axis=0
    ).sort_values("id").drop(columns=["id"]).values
    labels = pd.concat(
        [label_train, label_val, feat_test[["id"]]], axis=0
    ).sort_values("id").fillna(-1)["0"].values
...
def clean_edges(edges):
    ...
    years = pd.concat(
        [year_train, year_val, year_test], axis=0
